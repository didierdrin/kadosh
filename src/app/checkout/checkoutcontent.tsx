...
export default function CheckoutContent() {
  ...
  useEffect(() => {
    ...
    if (productData) {
      ...
      setCartItems(items);
      setMainImage(items[0].img[0]); // Set the first image of the first item as the main image
...
async function sendPurchaseEmail(order: any) {
  if (user && user.email) { // Ensure user and email exist
    try {
      const response = await fetch('/api/pendingorderapi', {
        method: 'POST',
  ...
  const handleSubmit = async (e: React.FormEvent | null, paymentDetails: any = null) => {
    ...
    if (user) {
      ...
      try {
        ...
        // Add the order to the user's pending orders
      await updateDoc(userDocRef, {
        pending_orders: arrayUnion(order),
      });
            ...
            <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
              ...
              {cartItems.map((item) => (
                <div key={item.id} className="mb-4">
                  ...
                  <p className="text-gray-600">Price: RWF {item.price? item.price.toFixed(2) : "N/A"}</p>
                </div>
              ))}
              <div className="mb-4">
